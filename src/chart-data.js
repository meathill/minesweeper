...
export function transferEventsToData({
  events,
  efficiencyEvents = [],
  startTimeStamp,
}) {
  ...
  const maxTimestamp = allTimestamps.length
  ...
  const buckets = Array.from({ length: bucketCount + 1 }, (_, i) => ({
    interval: isMinuteScale
      ? Math.round(((i * bucketWidthSec) / 60) * 10) / 10
      : i * bucketWidthSec,
    open: 0,
    openBlank: 0,
    openSave: 0,
  ...
  events.forEach((event) => {
    ...
    const idx = Math.max(
      0,
      Math.min(bucketCount, Math.ceil(elapsedSec / bucketWidthSec)),
    );
    buckets[idx][event.type] = (buckets[idx][event.type] || 0) + 1;
  });

  // --- 逐操作精确点：效率与地雷概率 x 为真实时刻 ---
  const effPoints = efficiencyEvents
    .map((ev, idx) => {
      const sec =
        ev.timeSinceStartSec ?? (ev.clickTimestamp - startTimeStamp) / 1000;
  ...
  const flagPoints = events
    .filter((e) => e.type === 'flag')
    .map((e) => {
      const sec =
        e.timeSinceStartSec ?? (e.clickTimestamp - startTimeStamp) / 1000;
      return {
        x: getXForSec(sec),
  ...
  const openEvents = events
    .filter(
      (e) =>
        ['open', 'openBlank', 'openSave'].includes(e.type) && e.index != null,
    )
  ...
  for (const e of openEvents) {
    ...
    if (
      last &&
      last.index === e.index &&
      e.clickTimestamp - last.clickTimestamp < 50
    )
      continue;
    dedupEvents.push(e);
  }
