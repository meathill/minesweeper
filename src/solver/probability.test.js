...
function buildHardMidBoard() {
  ...
  const safes = [];
  for (let i = 0; i < total; i++)
    if (!isBomb[i] && !grid[i].isOpen) safes.push(i);
  shuffle(safes);
  for (let i = 0; i < Math.min(100, safes.length); i++)
    grid[safes[i]].isOpen = true;
  const unopened = [];
...
describe('computeProbabilities - 50/50 & overlapping', () => {
  ...
  it('经典 1-2-1 横向：两端必雷中间必安全 (简化版)', () => {
    ...
    const g = makeGrid(2, 3, (grid, row, col) => {
      grid[idx(0, 0, col)].isOpen = true;
      grid[idx(0, 0, col)].count = 1;
      grid[idx(0, 1, col)].isOpen = true;
      grid[idx(0, 1, col)].count = 2;
      grid[idx(0, 2, col)].isOpen = true;
  ...
  it('你提到的 2-2-1 纵向（含对角）=> 枚举验证', () => {
    ...
    const g = makeGrid(3, 4, (grid, row, col) => {
      for (let r = 0; r < 3; r++)
        for (let c = 1; c < 4; c++) grid[idx(r, c, col)].isOpen = true;
      grid[idx(0, 1, col)].count = 2;
      grid[idx(1, 1, col)].count = 2;
      grid[idx(2, 1, col)].count = 1;
      for (let r = 0; r < 3; r++)
        for (let c = 2; c < 4; c++) grid[idx(r, c, col)].count = 0;
    });
...
describe('computeProbabilities - 多分量与孤立格', () => {
  it('两个独立约束互不影响', () => {
    ...
    const g = makeGrid(5, 5, (grid, row, col) => {
      ...
      grid[idx(1, 1, col)].isOpen = true;
      grid[idx(1, 1, col)].count = 1;
      // 右下区域
      grid[idx(3, 3, col)].isOpen = false;
      grid[idx(4, 4, col)].isOpen = false;
      grid[idx(3, 4, col)].isOpen = true;
      grid[idx(3, 4, col)].count = 1;
      // 其他 count 0
      for (let i = 0; i < 25; i++)
        if (![idx(1, 1, col), idx(3, 4, col)].includes(i))
          grid[i].count = grid[i].isOpen ? 0 : 0;
      grid[idx(1, 1, col)].count = 1;
      grid[idx(3, 4, col)].count = 1;
    });
...
describe('computeProbabilities - 边界与近似', () => {
  ...
  it('超大分量 >25 触发近似', () => {
    ...
    const g = makeGrid(row, col, (grid) => {
      ...
      // 使其他数字不产生约束
      for (let c = 0; c < col; c++)
        if (c !== 13) grid[idx(0, c, col)].count = 0;
    ...
    const g2 = makeGrid(3, 10, (grid, r, c) => {
      for (let cc = 0; cc < c; cc++) {
        grid[idx(0, cc, c)].isOpen = true;
        grid[idx(0, cc, c)].count = 2;
      }
      for (let cc = 0; cc < c; cc++) {
        grid[idx(1, cc, c)].isOpen = false;
      }
      for (let cc = 0; cc < c; cc++) {
        grid[idx(2, cc, c)].isOpen = true;
        grid[idx(2, cc, c)].count = 0;
      }
    });
  ...
  it('矛盾盘面（无解）回退近似不崩', () => {
    ...
    const g = makeGrid(2, 2, (grid, col) => {
      grid[idx(0, 0, 2)].isOpen = true;
      grid[idx(0, 0, 2)].count = 2;
      grid[idx(0, 1, 2)].isOpen = false; // 仅 1 未知但 need 2
      grid[idx(1, 0, 2)].isOpen = true;
      grid[idx(1, 0, 2)].count = 0;
      grid[idx(1, 1, 2)].isOpen = true;
      grid[idx(1, 1, 2)].count = 0;
    });
...
describe('getBestProbs / scoreForAction（固定差距扣分）', () => {
  ...
  it('顺利局（全盘概率都低）不再被系统性打低分', () => {
    ...
    assert.ok(
      approxEqual(
        scoreForAction({ prob: 0.05, pMin: 0, pMax: 0.15, action: 'flag' }),
        0.8,
      ),
    );
    ...
    assert.ok(
      approxEqual(
        scoreForAction({ prob: 0, pMin: 0, pMax: 0.15, action: 'flag' }),
        0.7,
      ),
    );
    // 点 prob=0.1 的格子
    assert.ok(
      approxEqual(
        scoreForAction({ prob: 0.1, pMin: 0, pMax: 0.15, action: 'open' }),
        0.8,
      ),
    );
  });
  ...
  it('危险局点明显更差的格子仍然得低分', () => {
    ...
    assert.ok(
      approxEqual(
        scoreForAction({ prob: 0.6, pMin: 0.3, pMax: 0.9, action: 'open' }),
        0.4,
      ),
    );
    // 有必雷格（0.9）不插，插在最安全的 0.3 上 => 0 分
    assert.equal(
      scoreForAction({ prob: 0.3, pMin: 0.3, pMax: 0.9, action: 'flag' }),
      0,
    );
    // 拔掉几乎必雷的旗 => 0 分；拔掉最安全的旗 => 满分
    assert.equal(
      scoreForAction({ prob: 0.9, pMin: 0.3, pMax: 0.9, action: 'unflag' }),
      0,
    );
    assert.equal(
      scoreForAction({ prob: 0.3, pMin: 0.3, pMax: 0.9, action: 'unflag' }),
      1,
    );
  });
  ...
  it('等价局（所有选项概率相同）不扣分', () => {
    assert.equal(
      scoreForAction({ prob: 0.5, pMin: 0.5, pMax: 0.5, action: 'open' }),
      1,
    );
    assert.equal(
      scoreForAction({ prob: 0.5, pMin: 0.5, pMax: 0.5, action: 'flag' }),
      1,
    );
  });
  ...
  it('与最优差距满 0.5 扣满，超出 clamp 到 0', () => {
    ...
    assert.equal(
      scoreForAction({ prob: 0.6, pMin: 0.1, pMax: 1, action: 'open' }),
      0,
    );
    // flag：pMax - prob = 0.8 => clamp 0
    assert.equal(
      scoreForAction({ prob: 0.2, pMin: 0, pMax: 1, action: 'flag' }),
      0,
    );
  });
  ...
  it('最优本身得满分', () => {
    assert.equal(
      scoreForAction({ prob: 1, pMin: 0, pMax: 1, action: 'flag' }),
      1,
    );
    assert.equal(
      scoreForAction({ prob: 0, pMin: 0, pMax: 1, action: 'open' }),
      1,
    );
  });
  ...
  it('非法输入返回 null', () => {
    assert.equal(
      scoreForAction({ prob: null, pMin: 0, pMax: 1, action: 'open' }),
      null,
    );
    assert.equal(
      scoreForAction({ prob: 0.5, pMin: null, pMax: 1, action: 'flag' }),
      null,
    );
    assert.equal(
      scoreForAction({ prob: 0.5, pMin: 0, pMax: null, action: 'flag' }),
      null,
    );
  });
});
...
describe('computeProbabilities - 大分量性能与 forced 回归', () => {
  it('子集规则直接解掉大分量链条：无需搜索，无平均，无近似标记', () => {
    ...
    const { map, isApproximate, decidedSet, frontierSet } =
      computeProbabilities(g, 2, col, 20);
  ...
  it('decidedSet 在真实 Hard 盘上是真子集（近似格不在列）', () => {
    const { grid, row, col, bombs } = buildHardMidBoard();
    const { decidedSet, frontierSet } = computeProbabilities(
      grid,
      row,
      col,
      bombs,
...
describe('createForcedRefiner - 后台精算', () => {
  ...
  it('分片推进与一次跑完结果一致（cursor 纪律）', () => {
    ...
    const loose = makeGrid(3, col, (grid) => {
      for (let c = 0; c < col; c++) {
        grid[idx(1, c, col)].isOpen = true;
        grid[idx(1, c, col)].count = c % 2 === 0 ? 1 : 2;
      }
      for (let c = 0; c < col; c++) {
        grid[idx(0, c, col)].isOpen = false;
        grid[idx(2, c, col)].isOpen = false;
      }
    });
    const drain = (fuel) => {
    ...
    assert.deepEqual(
      [...tiny.entries()].sort((a, b) => a[0] - b[0]),
      [...huge.entries()].sort((a, b) => a[0] - b[0]),
    );
  });
  ...
  it('小分量无需精算（total 为 0），skipSet 能跳过已定格', () => {
    ...
    const small = makeGrid(2, 3, (grid) => {
      grid[idx(0, 0, 3)].isOpen = true;
      grid[idx(0, 0, 3)].count = 1;
      grid[idx(0, 1, 3)].isOpen = true;
      grid[idx(0, 1, 3)].count = 2;
      grid[idx(0, 2, 3)].isOpen = true;
      grid[idx(0, 2, 3)].count = 1;
    });
...
describe('computeProbabilities - 大分量无 forced 不丢格（前沿灰带回归）', () => {
  it('单约束大分量无 forced：solveLarge 返回空但 hasForced 为 false', () => {
    ...
    const large = solveLargeComponentExport(
      vars,
      [{ vars: [...vars], need: 15 }],
      100000,
    );
  ...
  it('大分量无 forced 时前沿格全员仍有近似概率（截图灰带回归）', () => {
    ...
    const { map, isApproximate, frontierSet } = computeProbabilities(
      g,
      3,
      col,
      15,
