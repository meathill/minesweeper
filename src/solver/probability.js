...
function propagateConstraints(constraints) {
  ...
  for (const c of active)
    needByVars.set([...c.vars].sort((a, b) => a - b).join(','), c.need);
  let changed = true;
  let rounds = 0;
  while (changed && rounds++ < PROPAGATE_MAX_ROUNDS) {
    ...
    for (const c of active) {
      ...
      for (const v of c.vars) {
        if (decided.has(v)) {
          if (decided.get(v) === 1) need--;
        } else vars.push(v);
      }
      if (need < 0 || need > vars.length) {
        contradiction = true;
        continue;
      } // 脏约束，丢弃
      ...
      for (let i = 0; i < m; i++) {
        ...
        for (let j = i + 1; j < m; j++) {
          ...
          for (const [vars, need] of regions) {
            ...
            const sortedKey = vars
              .slice()
              .sort((a, b) => a - b)
              .join(',');
            const prev = needByVars.get(sortedKey);
            if (prev !== undefined && prev !== need) {
              contradiction = true;
              continue;
            }
...
function enumerateComponent(vars, constraints, budget) {
  ...
  for (let newPos = 0; newPos < n; newPos++)
    ordered[order[newPos]] = mineCounts[newPos];
...
export function hasSolutionWithFixedExport(
  vars,
  constraints,
  pos,
  val,
  nodeBudget = CALL_NODE_BUDGET,
) {
  return hasSolutionWithFixed(vars, constraints, pos, val, {
    remaining: nodeBudget,
  });
}
export function solveLargeComponentExport(
  vars,
  constraints,
  nodeBudget = CALL_NODE_BUDGET,
) {
...
function solveLargeComponent(vars, constraints, budget) {
  ...
  for (let si = 0; si < n; si++) {
    ...
    const q1 =
      q0 === 'sat'
        ? hasSolutionWithFixed(sortedVars, constraints, si, 1, quickBudget)
        : 'unknown';
    if (q0 === 'sat' && q1 === 'sat') continue;
    const canBe0 =
      q0 === 'sat'
        ? 'sat'
        : hasSolutionWithFixed(sortedVars, constraints, si, 0, budget);
    const canBe1 =
      q1 === 'sat'
        ? 'sat'
        : hasSolutionWithFixed(sortedVars, constraints, si, 1, budget);
    if (canBe0 === 'unsat' && canBe1 !== 'unsat') {
      forced.set(sortedVars[si], 1);
      hasForced = true;
    } else if (canBe0 !== 'unsat' && canBe1 === 'unsat') {
      forced.set(sortedVars[si], 0);
      hasForced = true;
    } else if (canBe0 === 'unsat' && canBe1 === 'unsat') {
      // 无解，脏数据，回退
...
export function createForcedRefiner(grid, row, column, skipSet = new Set()) {
  ...
  const { decided: propDecided, constraints: simplified } =
    propagateConstraints(rawConstraints);
  const fresh = new Map();
  ...
  function pendingCount() {
    let n = 0;
    for (let j = jobIdx; j < jobs.length; j++)
      n += jobs[j].pending.length - jobs[j].cursor;
    return n;
  }
  return {
    total,
    ...
    step({ fuelNodes = 80000, wallMs = 24 } = {}) {
      ...
      while (jobIdx < jobs.length) {
        ...
        while (job.cursor < job.pending.length) {
          ...
          if (
            progressed &&
            (fuel.remaining <= 0 || performance.now() - t0 > wallMs)
          ) {
            return { newly, done: false };
          }
          const pi = job.pending[job.cursor];
          // 快筛不计 fuel（上限千级节点总是跑完）：两侧易 sat 则必非 forced，直接跳过
          const q0 = hasSolutionWithFixed(job.vars, job.constraints, pi, 0, {
            remaining: REFINE_QUICK_NODES,
          });
          const q1 =
            q0 === 'sat'
              ? hasSolutionWithFixed(job.vars, job.constraints, pi, 1, {
                  remaining: REFINE_QUICK_NODES,
                })
              : 'unknown';
          if (q0 === 'sat' && q1 === 'sat') {
          ...
          const canBe0 =
            q0 === 'sat'
              ? 'sat'
              : hasSolutionWithFixed(job.vars, job.constraints, pi, 0, b0);
          fuel.remaining -= REFINE_PER_CHECK_NODES - b0.remaining;
          const b1 = { remaining: REFINE_PER_CHECK_NODES };
          const canBe1 =
            q1 === 'sat'
              ? 'sat'
              : hasSolutionWithFixed(job.vars, job.constraints, pi, 1, b1);
          fuel.remaining -= REFINE_PER_CHECK_NODES - b1.remaining;
          job.cursor++;
          progressed = true;
          if (canBe0 === 'unsat' && canBe1 !== 'unsat')
            newly.set(job.vars[pi], 1);
          else if (canBe0 !== 'unsat' && canBe1 === 'unsat')
            newly.set(job.vars[pi], 0);
...
export function computeProbabilities(grid, row, column, bombNumber) {
  ...
  for (const comp of components) {
    ...
    const { total, mineCounts, aborted } = enumerateComponent(
      comp.vars,
      comp.constraints,
      budget,
    );
    if (aborted) {
      ...
      isApproximate = true;
      const large = solveLargeComponent(comp.vars, comp.constraints, budget);
      const probs = large.hasForced
        ? large.forced
        : approximateComponent(comp.vars, comp.constraints);
